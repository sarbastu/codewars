...
function mxdiflg(a1, a2) {
  ...
  const arr1ItemLengths = a1.map((e) => e.length);
  const arr2ItemLengths = a2.map((e) => e.length);

  const firstMax = Math.max(...arr1ItemLengths) - Math.min(...arr2ItemLengths);
  const secondMax = Math.max(...arr2ItemLengths) - Math.min(...arr1ItemLengths);
...
function mxdiflg(a1, a2) {
  var max = -1;
  for (var A1 of a1)
    for (var A2 of a2) max = Math.max(max, Math.abs(A1.length - A2.length));
  return max;
}
